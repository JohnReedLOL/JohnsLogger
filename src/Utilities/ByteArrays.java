...
public final class ByteArrays {
    ...
    public static <Type> Type tryDeepCopy(Type toClone) {
        try {
            ...
            AppTester.check(toClone instanceof Serializable, "Only serializable objects can be deep copied.");
            ...
            // AppTester.check(toReturn instanceof Type); // Doesn't work
            final Type toReturnCast = (Type) toReturn; // unsafe operation.
            return toReturnCast; 
        } catch(InvalidClassException ice) {
            AppTester.printEx("Something is wrong with the class", ice);
            return null;
        } catch (IOException ioe) {
            AppTester.printEx("Something is blocking this byte array read/write operation", ioe);
            return null;
        } catch (ClassNotFoundException e) {
            AppTester.killApplication("You have to find this class because it was the same type going in as going out.");
            return null;
        } catch (Exception e) {
            AppTester.printEx(e);
            return null;
    ...
    private static long byteArraySegmentToLong(byte[] by, int start) {
        ...
        AppTester.check(bytesToMakeIntoALong.length == 8, "Each long in is eight bytes");
    ...
    public static long[] EntireByteArrayToMultipleLongs(byte[] bytes) {
        try {
            AppTester.check(bytes.length % 8 == 0, "Each long is 8 bytes, so the byte array must be a multiple of 8");
    ...
    public static byte[] nonPrimitiveArrayObjectToByteArray(final Object object) {
        /*if (object instanceof byte[]) {
         return (byte[]) object; // byte[] extends java.lang.Object
         }*/
        AppTester.check(!(object instanceof byte[]));
        AppTester.check(!(object instanceof short[]));
        AppTester.check(!(object instanceof int[]));
        AppTester.check(!(object instanceof long[]));
